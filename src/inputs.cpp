...
std::string rcli::ask_input_value(InputConfig config) {
    ...
    if (config._default != "" && value.empty()) value = config._default;
...
std::string rcli::ask_value_in_list(
    InputConfig config, std::vector<std::string> options, bool ignore_case) {
